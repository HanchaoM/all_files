function Auu = FAuu(M,k,x,QQ,t,phi)
...
if(isa(x,'double') && isa(phi,'double'))
 ...
 for i = 1:row
    for j = 1:col
        ...
        cth = -1 / sqrt( 1 + gg ) * ( 1 + gg / 2 * ( 1+ t / QQ ) / ( 1 + x(i,j) * t / QQ ) );
        ...
        Auu(i,j) = -4  / (KQP * KPQP) * ( ( QQ + t ) * (2 * ( KPPP + KPPPP ) * KK_T   + ( PPPQ * KQP_T ) + 2* ( KPPPP * KQP ) - 2* ( KPPP * KPQP ) ) + ( QQ - t + 4* KD ) * PPPQP * ( KKP_T + KQP_T - 2* KKP ) );
    end
end
